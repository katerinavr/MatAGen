...
class ExsclaimTool(ABC):
    ...
    def initialize_query(self, search_query):
        ...
        current_dir = Path(search_query.get("output_dir")) #Path.cwd()
        # base_results_dir = paths_drop.initialize_results_dir(
        #     self.search_query.get("results_dirs", None)
        # )
        self.results_directory = current_dir / self.search_query["name"]
        print('self.results_directory_tool', self.results_directory)
...
class PDFScraper(ExsclaimTool):
    ...
    def __init__(self, search_query, driver=None):
        ...
        #     self.search_query.get("results_dirs", None)
        # )
        current_dir = Path(search_query.get("output_dir"))
        # current_dir =  search_query.get("output_dir", False) #Path.cwd()
        self.results_directory = current_dir / self.search_query["name"]
        figures_directory = self.results_directory / "figures"
        os.makedirs(figures_directory, exist_ok=True)
    ...
    def read_figure_captions(self,img_path, api_key ):
        ...
        try:         
            ...
            print("Exception in read_figure_captions:", e)
            # Return an empty JSON string instead of None
            return "{}" 
...
class CaptionDistributor(ExsclaimTool):
    ...
    def _update_exsclaim(self,search_query,  exsclaim_dict, figure_name, delimiter, caption_dict):
        ...
        caption_dict = caption_dict or {} # Ensure caption_dict is not None
