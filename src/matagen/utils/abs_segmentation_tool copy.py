...
# Go up levels from script_dir to reach the directory containing 'checkpoints'
# Adjust the number of '..' based on where this script is relative to project root!
project_root = os.path.abspath(os.path.join(script_dir, '..', '..', '..')) # e.g., if script is 3 levels down from root
checkpoints_base = os.path.join(project_root, 'checkpoints', 'axis_alignment') # Assumes checkpoints is directly under project_root
checkpoints_plot_extract_base = os.path.join(project_root, 'checkpoints', 'plot_data_extraction') # Base for these files

# Configuration dictionaries adapted from https://github.com/MaterialEyes/Plot2Spec
axis_align_opt = {
    # region detection
    "config_file": os.path.join(checkpoints_base, "fcos_x101_64x4d_fpn_gn-head_mstrain_640-800_2x_coco.py"),
    "checkpoint_file": os.path.join(checkpoints_base, "epoch_200.pth"), #"checkpoints/axis_alignment/epoch_200.pth",
    "refinement": True,
    # tick detection
    "cuda": False,
    "canvas_size": 1280,
    ...
    "show_time": False,
    "refine": True,
    "trained_model": os.path.join(checkpoints_base, "craft_mlt_25k.pth"), #'checkpoints/axis_alignment/craft_mlt_25k.pth',
...
background_opt = {
    ...
    "mode": "abs_spectra",
    "num_workers": 0,
    "model_file":os.path.join(checkpoints_plot_extract_base, "lanenet", "deeplab_all_checkpoint", "0999_checkpoint.ckpt"),
...
def get_abs_dictionary(path, abs_image, image_path):
...
if __name__ == "__main__":
    ...
    final_dict = get_abs_dictionary(image_path)
